          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- DO_NOTHING: translated text（translated text） -->
                ...
                <!-- translated text：translated text content translated text -->
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // fromStep2translated text
  minutesPerRound: {
    type: Number,
    default: 30 // Default every round30minute
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // translated text（translated text）
const actionIds = ref(new Set()) // translated textIDtranslated text
...
// translated text（Calculated based on rounds and minutes per round）
...
const doStartSimulation = async () => {
  ...
  // translated text，Make sure you are not affected by the previous simulation
  resetAllState()
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // translated text
    }
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // translated text（pass runner_status translated text）
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // translated text：translated text runner_status，translated text
      // translated text twitter_completed and reddit_completed translated text
...
const checkPlatformsCompleted = (data) => {
  ...
  // pass actions_count translated text（translated text count > 0 or running translated text true）
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // use all_actions translated text
