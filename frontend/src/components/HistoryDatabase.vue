<template>
  ...
  >
    <!-- translated text：translated text（Show only when there are items） -->
    ...
    <!-- card container（Show only when there are items） -->
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- card header：simulation_id and translated text -->
        ...
        <div class="card-files-wrapper">
          <!-- corner decoration - translated text -->
        ...
        <!-- card title（translated text20translated text） -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Card description（translated text） -->
        ...
        <!-- Bottom decorative line (hovertranslated text) -->
...
const selectedProject = ref(null)  // translated text（translated text）
let observer = null
let isAnimating = false  // translated text，translated text
let expandDebounceTimer = null  // translated text
let pendingState = null  // translated text
...
const containerStyle = computed(() => {
  ...
  // translated text：Number of lines * translated text + (Number of lines-1) * translated text + translated text
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    // expanded state：translated text
    ...
    const x = offset * 35
    // translated text，Close to title but keep proper spacing
...
// Format time（translated text:point）
...
// translated text（translated text20translated text）
...
// translated text simulation_id translated text（translated text6translated text）
...
const loadHistory = async () => {
  try {
    ...
    console.error('translated text:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // translated text（Record the latest target state regardless of whether it is in animation or not）
        ...
        // If the target state is the same as the current state，translated text
        ...
        // translated text，translated text
        // translated text(50ms)，translated text(200ms)translated text
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Check whether the pending execution status still needs to be executed（translated text）
          ...
          // translated text，and check if there are any pending status changes
          setTimeout(() => {
            isAnimating = false
            
            // translated text，Check if there is a new pending execution status
    ...
    },
    {
      ...
      // Adjustment rootMargin，translated text，translated text
...
// translated text，Reload data when returning to homepage
...
onMounted(async () => {
  // translated text DOM translated text
  await nextTick()
  await loadHistory()
  
  // wait DOM translated text
  setTimeout(() => {
    initObserver()
  }, 100)
})

// translated text keep-alive，Reload data on component activation
onActivated(() => {
  loadHistory()
})
...
.cards-container {
  ...
  /* min-height translated text JS translated text，translated text */
}
...
/* translated text */
.status-icon:nth-child(1).available { color: #3B82F6; } /* Map construction - translated text */
...
.card-progress.not-started { color: #9CA3AF; }  /* Not started - translated text */
...
/* translated text - Moranditranslated text */
...
/* Progress status color - translated text */
