<template>
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <!-- translated text (Internal Top Right) -->
...
const showEdgeLabels = ref(true) // translated text
const expandedSelfLoops = ref(new Set()) // translated text
const showSimulationFinishedHint = ref(false) // Prompt after simulation ends
const wasSimulating = ref(false) // translated text
...
// translated text isSimulating translated text，translated text
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Change from simulation to non-simulation state，translated text
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // translated text
}
...
const renderGraph = () => {
  ...
  // translated text，Calculate the number and index of edges between the same pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} // translated text
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count the number of edges between each pair of nodes，translated text
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Self-loop - translated text
  ...
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // translated text
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // translated text - Only one merged self-loop is added to each node
    ...
    // Determine whether the direction of the edge is consistent with the normalized direction（translated textUUID < translated textUUID）
    ...
    if (totalCount > 1) {
      // translated text，translated text
      // The curvature range increases based on the number of edges，translated text
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If the direction of the edge is opposite to the normalized direction，translated text
      // This ensures that all edges are distributed in the same reference frame，translated text
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      ...
      // translated text 150，Each additional side increases 40
    ...
    // translated text，Gather independent clusters of nodes into a central area
  ...
  // Links - use path translated text
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // translated text，translated text
      const x1 = sx + 8  // translated text
      const y1 = sy - 4
      const x2 = sx + 8  // translated text
      const y2 = sy + 4
      // translated text（sweep-flag=1 translated text）
    ...
    // translated text - Dynamically adjust based on edge number and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // translated text，translated text，translated text
    // translated text，The larger the offset is as a proportion of the distance
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // translated text25%，Each additional side increases5%
  ...
  const getLinkMidpoint = (d) => {
    ...
    // translated text B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, t=0.5
  ...
  // Link labels background (translated text)
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label position（translated text，translated text）
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Remove rotation，translated text
    })
