<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <!-- Step 02: generate Agent Personality -->
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Simulation round configuration - Only displayed after configuration generation is complete and rounds calculated -->
...
const props = defineProps({
  simulationId: String,  // translated text
  projectData: Object,
  graphData: Object,
  systemLogs: Array
})
...
// State
const phase = ref(0) // 0: initialization, 1: translated text, 2: Generate configuration, 3: Finish
...
// translated text：Record the key information of the last output
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Simulation round configuration
const useCustomRounds = ref(false) // translated text
const customMaxRounds = ref(40)   // translated text40wheel
...
watch(currentStage, (newStage) => {
  if (newStage === 'generateAgentPersonality' || newStage === 'generating_profiles') {
    ...
    phase.value = 2 // translated text
  }
})

// Calculate automatically generated rounds from configuration（translated text）
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // translated text null
  }
  ...
  // translated text40（translated text），translated text
...
// translated textagent_idtranslated textusername
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // translated text，translated text max_rounds parameter
    params.maxRounds = customMaxRounds.value
    addLog(t('log.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // User chooses to keep the automatically generated number of rounds，translated text max_rounds parameter
...
const startPrepareSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // translated textAgenttotal（frompreparetranslated text）
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // translated text（avoid duplication）
    ...
    console.warn('translated text:', err)
  }
}
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // translated text API translated text，translated text
      ...
      // translated text Profile translated text（translated text）
    ...
    console.warn('get Profiles fail:', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    console.warn('get Config fail:', err)
  }
}
