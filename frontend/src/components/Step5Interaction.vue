...
const chatHistoryCache = ref({}) // translated text: { 'report_agent': [], 'agent_0': [], 'agent_1': [], ... }
...
const selectReportAgentChat = () => {
  ...
  // translated text Report Agent s conversation record
...
const selectAgent = (agent, idx) => {
  ...
  // translated text Agent s conversation record
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? 'translated text' : 'translated text'}：${msg.content}`)
      .join('\n')
  ...
  if (res.success && res.data) {
    ...
    // Format: {"twitter_0": {...}, "reddit_0": {...}} translated text {"reddit_0": {...}}
    const resultData = res.data.result || res.data
    const resultsDict = resultData.results || resultData
    
    // translated text，translated text reddit translated text
...
/* Left Panel - Report Style (and Step4Report.vue translated text) */
.left-panel.report-style {
  width: 45%;
  min-width: 450px;
  background: #FFFFFF;
...
/* translated text - use CSS translated text ol translated text */
.message-text {
