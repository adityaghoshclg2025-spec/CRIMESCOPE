    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - translated text -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (Content generation completed，translated text) -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // wait DOM translated text，Adjust scroll position to keep buttons in the same position
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // translated text - translated text
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // translated text - translated text
  },
  'interview_agents': {
    name: 'Agent Interview',
    color: 'green',
    icon: 'users' // translated text - translated text
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // translated text - translated text
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // translated text - translated text
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // translated text - translated text
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // translated text
    const queryMatch = text.match(/translated text:\s*(.+?)(?:\n|$)/)
    ...
    // translated text - Complete extraction，No limit on quantity
    const subQSection = text.match(/### translated text\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    // translated text - Complete extraction，No limit on quantity
    const factsSection = text.match(/### 【translated text】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    // translated text - Complete extraction，translated text
    ...
    if (entitySection) {
      ...
      result.entities = entityBlocks.map(block => {
        ...
        const summaryMatch = block.match(/translated text:\s*"?(.+?)"?(?:\n|$)/)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract statistics
    const nodesMatch = text.match(/translated text:\s*(\d+)/)
    const edgesMatch = text.match(/translated text:\s*(\d+)/)
    ...
    // translated text/expired fact - Complete extraction，No limit on quantity
...
const parseInterview = (text) => {
  ...
  try {
    // translated text
    const topicMatch = text.match(/\*\*translated text:\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    // translated text（like "5 / 9 translated textAgent"）
    const countMatch = text.match(/\*\*translated text:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    // translated text
    const reasonMatch = text.match(/### translated text\n([\s\S]*?)(?=\n---\n|\n### translated text)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // For example: 1. **translated text_345（index=1）**：translated text...
        ...
        // Format2: - translated text（index X）：reason
        // For example: - translated text_601（index 0）：As a parent group representative...
        if (!headerMatch) {
        ...
        } else if (currentName && line.trim() && !line.match(/^translated text|^translated text|^translated text/)) {
          // translated text（translated text）
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // translated text（like "translated text"、"translated text" wait）
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // translated text"1."，translated text
      ...
      // translated text - pointTwitterandReddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quotes|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // translated textTwitterandRedditanswer
        ...
        if (!twitterMatch && redditMatch) {
          // translated text Reddit answer，Copy as default display only when non-placeholder text
      ...
      if (quotesMatch) {
        ...
        // translated text > "text" Format
        let quoteMatches = quotesText.match(/> "([^"]+)"/g)
        // translated text：match > "text" or > \u201Ctext\u201D（translated text）
        if (!quoteMatches) {
    ...
    // translated text
    const summaryMatch = text.match(/### translated text\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // translated text（if there is）
    const edgesSection = text.match(/### translated text:\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // translated text-Answer to Maintaining independent platform selection status
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（The platform did not receive a reply）' || t === '(The platform did not receive a reply)' || t === '[translated text]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // translated text：
      // 1. "questionX：" or "questionX:" （translated text，translated text）
      // 2. "1. " or "\n1. " （number+translated text，translated text）
      let matches = []
      let match

      // translated text "questionX：" Format
      const cnPattern = /(?:^|[\r\n]+)question(\d+)[：:]\s*/g
      ...
      // translated text，translated text "number." Format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no number is found or only one is found，translated text
      if (matches.length <= 1) {
    ...
    // Check whether a question has dual-platform answers（translated text）
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - translated text
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // translated text（Only shown if there are real dual-platform answers）
                  ...
                  // Expand/Collapse Button（translated text）
...
const activeStep = computed(() => {
  ...
  // translated text active steps
  const active = steps.find(s => s.status === 'active')
  if (active) return active
  
  // translated text active，translated text done steps
  const doneSteps = steps.filter(s => s.status === 'done')
  if (doneSteps.length > 0) return doneSteps[doneSteps.length - 1]
  
  // translated text
  return steps[0] || { noLabel: '--', title: 'translated text', status: 'todo', meta: '' }
})
...
const renderMarkdown = (content) => {
  ...
  // translated text（## xxx），Because the chapter title is already displayed in the outer layer
  ...
  // translated text - translated text，translated text <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
...
const getLogLevelClass = (log) => {
  ...
  if (log.includes('WARNING') || log.includes('translated text')) return 'warning'
  // INFO translated text，translated text success
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // translated text loading state
            emit('update-status', 'completed')
            stopPolling()
            // The scrolling logic is unified after the loop ends nextTick translated text
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // translated text，translated text；Otherwise scroll to the bottom to follow the latest posts
            if (isComplete.value) {
...
// translated text - from LLM response translated text
const extractFinalContent = (response) => {
  if (!response) return null
  
  // translated text <final_answer> translated text
  ...
  // try to find Final Answer: What follows（translated text）
  ...
  // translated text ## or # or > translated text，translated text markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }
  
  // translated textmarkdownFormat，translated text
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // translated text Thought: translated text
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* translated text，translated text */
  font-weight: 500;
}
...
/* Console Logs - and Step3Simulation.vue translated text */
.console-logs {
  background: #000;
  color: #DDD;
  padding: 16px;
