      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">translated text</span>
          </div>
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? 'translated text' : 'translated text'">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Map visualization（translated text） -->
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">translated text...</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  translated text，LLMtranslated text，Automatically generate an ontology structure suitable for public opinion simulation（Entity type + translated text）
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">translated text ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">translated text ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} translated text...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Interface description</div>
                <div class="detail-content">
                  generation-based ontology，translated text Zep API Build knowledge graph，Extract entities and relationships
                </div>
              </div>
              
              <!-- translated text -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">translated text...</div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">translated text</div>
              </div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">translated text</span>
          </div>
          <div class="project-details" v-if="projectData">
            <div class="project-item">
              <span class="item-label">translated text</span>
...
// translated textID（translated text'new'translated textID）
...
const currentPhase = ref(-1) // -1: translated text, 0: Ontology is being generated, 1: Map construction, 2: Finish
const selectedItem = ref(null) // translated text
...
const goToNextStep = () => {
  ...
  router.push({
    name: 'Simulation',
    params: { simulationId: currentProjectId.value }
  })
}
...
const initProject = async () => {
  ...
  if (paramProjectId === 'new') {
    // translated text：from store translated text
...
// translated text - call ontology/generate API
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'translated text，translated text'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // translated text
    ontologyProgress.value = { message: 'translated text...' }
    ...
    if (response.success) {
      ...
      // translated textURL（translated text）
      router.replace({
        name: 'Process',
      ...
      error.value = response.error || 'translated text'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'translated text: ' + (err.message || 'unknown error')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || 'translated text'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      // save task_id translated text
      const taskId = response.data.task_id
      
      // Start graph data polling（translated text）
      startGraphPolling()
...
const startGraphPolling = () => {
  ...
  // translated text 10 translated text
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Map construction completed，translated text...')
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Build completed，translated text...'
        }
        ...
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 translated text:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'translated text: ' + (task.error || 'unknown error')
        buildProgress.value = null
      }
    }
  } catch (err) {
...
const renderGraph = () => {
  ...
  const nodes = nodesData.map(n => ({
    id: n.uuid,
    name: n.name || 'translated text',
    type: n.labels?.find(l => l !== 'Entity' && l !== 'Node') || 'Entity',
    rawData: n // translated text
  }))
  ...
  simulation.on('tick', () => {
    // translated text（Includes visible lines and transparent click areas）
