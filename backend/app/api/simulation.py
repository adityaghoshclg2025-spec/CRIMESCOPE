"""
 API 
Step2: Zep 、OASIS （Full automation）
"""
...
logger = get_logger('crimescope.api.simulation')


# Interview prompt  
#  AgentCall tool， 
INTERVIEW_PROMPT_PREFIX = " 、All past memories and actions， ："


def optimize_interview_prompt(prompt: str) -> str:
    """
    optimizationInterview ，Add prefix to avoidAgentCall tool
...
def create_simulation():
    ...
    Notice：max_rounds LLM ， 
    
    ask（JSON）：
        {
            "project_id": "proj_xxxx",      // Required
            "graph_id": "crimescope_xxxx",    // Optional， projectget
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    """
     ...
     ：
    1. state.json   status for "ready"
    2.  ：reddit_profiles.json, twitter_profiles.csv, simulation_config.json
    
    Notice： (run_*.py)  backend/scripts/ Table of contents，No longer copied to mock directory
    ...
    #  （ ，The script is located at backend/scripts/）
    required_files = [
        "state.json",
    ...
    try:
        ...
        # if config_generated=True  ， 
        #  ：
        # - ready:  ， 
        # - preparing: if config_generated=True  
        # - running:  ，It means that preparations have been completed long ago
        # - completed:  ，It means that preparations have been completed long ago
        # - stopped:  ，It means that preparations have been completed long ago
        # - failed:  （ ）
        ...
        if status in prepared_statuses and config_generated:
            ...
            return False, {
                "reason": f" config_generatedforfalse: status={status}, config_generated={config_generated}",
                "status": status,
                "config_generated": config_generated
            }
...
def prepare_simulation():
    ...
    Prepare simulation environment（ ，LLM ）
     ...
     ， task_id，
    use GET /api/simulation/prepare/status  
    ...
    ask（JSON）：
        {
            ...
            "parallel_profile_count": 5,                  // Optional， ，default5
    ...
    try:
        ...
        logger.info(f"  /prepare ask: simulation_id={simulation_id}, force_regenerate={force_regenerate}")
        
        #  （avoid duplicate generation）
        if not force_regenerate:
            logger.debug(f"  {simulation_id} Are you ready?...")
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            logger.debug(f" : is_prepared={is_prepared}, prepare_info={prepare_info}")
            if is_prepared:
                logger.info(f"simulation {simulation_id} Ready， ")
                return jsonify({
                    "success": True,
                    "data": {
                ...
                logger.info(f"simulation {simulation_id} Not ready yet， ")
        ...
        # ========== Get the number of entities synchronously（ ） ==========
        #  prepare Agent 
        try:
...
def get_prepare_status():
    ...
    ask（JSON）：
        {
            "task_id": "task_xxxx",          // Optional，prepare task_id
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    get simulation   report_id
      ...
      reports Table of contents，  simulation_id   report，
     （  created_at  ）
    ...
    try:
        ...
        logger.warning(f"  simulation {simulation_id} of report fail: {e}")
        return None
...
def get_simulation_history():
    ...
    try:
        ...
        for sim in simulations:
            ...
            #  （from simulation_config.json read simulation_requirement）
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
     Agent Profile（Used to view progress in real time during the build process）
    
    and /profiles The difference between interfaces：
    - Read files directly，  SimulationManager
    ...
    try:
        ...
        # Check if it is being generated（pass state.json  ）
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    """
     （Used to view progress in real time during the build process）
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation configuration（LLM ）
    
     ：
        - time_config: Time configuration（ 、 、 /Trough period）
        - agent_configs:  Agentactivity configuration（ 、speaking frequency、 ）
...
def generate_profiles():
    ...
    ask（JSON）：
        {
            "graph_id": "crimescope_xxxx",     // Required
...
def start_simulation():
    ...
    ask（JSON）：
        {
            ...
            "enable_graph_memory_update": false,   // Optional:  AgentActivity updates toZep 
            "force": false                         // Optional: force restart（ ）
        }

    about force parameter：
        -  ， ， 
        -  ：run_state.json, actions.jsonl, simulation.log wait
        -  （simulation_config.json）and profile document
        -  

    about enable_graph_memory_update：
        -  ， Agent （ 、Comment、 ） ZepAtlas
        -  " " ， AI 
        -   graph_id
        -  ， API 
    ...
    try:
        ...
        enable_graph_memory_update = data.get('enable_graph_memory_update', False)  # Optional： 
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                #  ， 
                if force:
                    logger.info(f"Forced mode：  {simulation_id}")
                ...
                logger.info(f"simulation {simulation_id} Preparation work completed，Reset status to ready（ : {state.status.value}）")
...
def interview_agents_batch():
    ...
    ask（JSON）：
        {
            ...
            "interviews": [                    // Required，Interview list
                {
                    "agent_id": 0,
                    "prompt": " AWhat do you think?？",
                    "platform": "twitter"      // Optional， Agent 
                },
                {
                    "agent_id": 1,
                    "prompt": " BWhat do you think?？"  // Not specifiedplatform 
                }
            ],
            "platform": "reddit",              // Optional，Default platform（ platformcover）
    ...
    try:
        ...
        #  prompt，Add prefix to avoidAgentCall tool
...
def interview_all_agents():
    ...
    ask（JSON）：
        {
            "simulation_id": "sim_xxxx",            // Required，simulationID
            "prompt": " ？",  // Required，interview questions（ AgentUse the same question）
    ...
    ask（JSON）：
        {
            ...
            "platform": "reddit",          // Optional，platform type（reddit/twitter）
                                           //  
            "agent_id": 0,                 // Optional，Get only theAgent 
