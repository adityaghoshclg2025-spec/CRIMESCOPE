...
logger = get_logger('crimescope.api.report')
...
def list_reports():
    ...
    Queryparameter：
        ...
        limit: Return quantity limit（default50）
...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # ifMDFile does not exist，Generate a temporary file
            import tempfile
...
def chat_with_report_agent():
    ...
    Report AgentSearch tools can be used to answer questions during conversations
    ...
    try:
        ...
        # createAgentand have a conversation
        agent = ReportAgent(
            graph_id=graph_id,
            simulation_id=simulation_id,
            simulation_requirement=simulation_requirement
...
def get_agent_log(report_id: str):
    ...
    - Report starts、Planning begins/Finish
    - Beginning of each chapter、Tool call、LLMresponse、Finish
    - Report completion or failure
    
    Queryparameter：
        from_line: Which line should I start reading from?（Optional，default0，for incremental acquisition）
    ...
    try:
        ...
        logger.error(f"getAgentLog failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_console_log(report_id: str):
    ...
    Get console output during report generation in real time（INFO、WARNINGwait），
