...
logger = get_logger('crimescope.simulation_runner')
...
class SimulationRunState:
    ...
    #  （  actions.jsonl   simulation_end event）
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        enable_graph_memory_update: bool = False,  #  ZepAtlas
        graph_id: str = None  # ZepAtlasID（Required when enabling map updates）
    ) -> SimulationRunState:
        """
        Start simulation
        ...
        Args:
            ...
            enable_graph_memory_update:  AgentActivity updates toZepAtlas
            graph_id: ZepAtlasID（Required when enabling map updates）
        ...
        if not os.path.exists(config_path):
            raise ValueError(f"Impersonation configuration does not exist，  /prepare interface")
        ...
        # If the maximum number of rounds is specified， 
        ...
        try:
            ...
            #  ，make sure Windows   UTF-8 coding
            #  （like OASIS） 
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support，  open()   UTF-8
                ...
                try:
                    if os.path.exists(main_log_path):
                        with open(main_log_path, 'r', encoding='utf-8') as f:
                            error_info = f.read()[-2000:]  #  2000character
                except Exception:
                    pass
                state.error = f" : {exit_code}, mistake: {error_info}"
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # /F = Forced termination, /T = Terminate process tree（ ）
            ...
            try:
                ...
                try:
                    ...
                    # Forced termination
                    logger.warning(f" ，Forced termination: {simulation_id}")
                    subprocess.run(
                ...
                logger.warning(f"taskkill fail，  terminate: {e}")
                process.terminate()
                try:
                    process.wait(timeout=5)
                except subprocess.TimeoutExpired:
            ...
            try:
                ...
                #  ，  SIGKILL
                logger.warning(f"  SIGTERM，Forced termination: {simulation_id}")
    ...
    def _read_actions_from_file(
        ...
        Args:
            file_path:  
            default_platform: Default platform（  platform  ）
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    #  （like simulation_start, round_start, round_end  ）
    ...
    def get_all_actions(
        ...
        # read Twitter  （Automatically set based on file path platform for twitter）
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        if not platform or platform == "twitter":
            actions.extend(cls._read_actions_from_file(
                twitter_actions_log,
                default_platform="twitter",  #   platform Field
                platform_filter=platform,
                agent_id=agent_id, 
                round_num=round_num
            ))
        
        # read Reddit  （Automatically set based on file path platform for reddit）
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        if not platform or platform == "reddit":
            actions.extend(cls._read_actions_from_file(
                reddit_actions_log,
                default_platform="reddit",  #   platform Field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            ))
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  #   platform Field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Notice： （simulation_config.json）and profile document
        ...
        for filename in files_to_delete:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    errors.append(f"  {filename} fail: {str(e)}")
            ...
            if os.path.exists(dir_path):
                ...
                if os.path.exists(actions_file):
                    try:
                        ...
                        errors.append(f"  {dir_name}/actions.jsonl fail: {str(e)}")
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        ...
                        logger.warning(f"  state.json fail: {simulation_id}, error={state_err}")
    ...
    def register_cleanup(cls):
        ...
        # Flask debug  ，only in reloader  （ ）
        # WERKZEUG_RUN_MAIN=true   reloader  
        #   debug model， ， 
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None
        
        # exist debug  ，only in reloader  ；No debug  
        ...
        # SIGHUP only in Unix  （macOS/Linux），Windows  
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
             ，Include status, twitter_available, reddit_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"The simulation environment is not running or is shut down， Interview: {simulation_id}")

        logger.info(f"sendInterviewOrder: simulation_id={simulation_id}, agent_id={agent_id}, platform={platform}")
    ...
    def interview_agents_batch(
        ...
        logger.info(f" InterviewOrder: simulation_id={simulation_id}, count={len(interviews)}, platform={platform}")
    ...
    def interview_all_agents(
        ...
        """
         Agent（global interview）

         Agent

        Args:
            simulation_id: simulationID
            prompt: interview questions（ AgentUse the same question）
            platform: designated platform（Optional）
                - "twitter": Interview onlyTwitterplatform
                - "reddit": Interview onlyRedditplatform
                - None: When simulating dual platforms, eachAgentInterview on two platforms at the same time
        ...
        logger.info(f" InterviewOrder: simulation_id={simulation_id}, agent_count={len(interviews)}, platform={platform}")
    ...
    def close_simulation_env(
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment shutdown command sent（Timeout waiting for response， ）"
            }
    ...
    def _get_interview_history_from_db(
        ...
        try:
            ...
            logger.error(f"readInterviewhistorical failure ({platform_name}): {e}")
    ...
    def get_interview_history(
        ...
        Args:
            ...
            platform: platform type（reddit/twitter/None）
                - "reddit":  RedditPlatform history
                - "twitter":  TwitterPlatform history
                - None:  
            agent_id:  Agent ID（Optional，Get only theAgent ）
            limit:  
