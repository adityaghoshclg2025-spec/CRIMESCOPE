...
logger = get_logger('crimescope.zep_graph_memory_updater')
...
class AgentActivity:
    ...
    def _describe_like_post(self) -> str:
        """  - Contains the original text of the post and author information"""
    ...
    def _describe_repost(self) -> str:
        ...
        if original_content and original_author:
            return f" {original_author}s posts：「{original_content}」"
        elif original_content:
            return f"retweeted a post：「{original_content}」"
        elif original_author:
            return f" {original_author}a post of"
    ...
    def _describe_quote_post(self) -> str:
        ...
        base = ""
        if original_content and original_author:
            base = f" {original_author}s posts「{original_content}」"
        elif original_content:
            base = f"quoted a post「{original_content}」"
        elif original_author:
            base = f" {original_author}a post of"
        else:
            base = "quoted a post"
    ...
    def _describe_create_comment(self) -> str:
        ...
        if content:
            if post_content and post_author:
                return f"exist{post_author}s posts「{post_content}」 ：「{content}」"
            elif post_content:
                return f" 「{post_content}」 ：「{content}」"
            elif post_author:
                return f"exist{post_author} ：「{content}」"
            return f" ：「{content}」"
        return " "
    
    def _describe_like_comment(self) -> str:
        """  - Contains review content and author information"""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
        
        if comment_content and comment_author:
            return f"Liked{comment_author} ：「{comment_content}」"
    ...
    def _describe_dislike_comment(self) -> str:
        ...
        if comment_content and comment_author:
            return f"stepped on{comment_author} ：「{comment_content}」"
        elif comment_content:
...
class ZepGraphMemoryUpdater:
    ...
    ZepMap memory updater
     ...
     Zep，action_argswill contain complete contextual information：
    - Like/ 
    - Forward/ 
    -  / 
    - Like/ 
    ...
    #  （Second），Avoid requesting too quickly
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._total_sent = 0        # Successfully sent toZep 
        self._total_items_sent = 0  # Successfully sent toZep 
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        - SEARCH_POSTS（ ）
        - SEARCH_USER（ ）
        - LIKE_POST/DISLIKE_POST（Like/ ）
        - REPOST（Forward）
        - FOLLOW（ ）
        - MUTE（ ）
        - LIKE_COMMENT/DISLIKE_COMMENT（Like/ ）
        
        action_argswill contain complete contextual information（ 、 ）。
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logger.info(f"  {len(activities)} strip{display_name}  {self.graph_id}")
                logger.debug(f" : {combined_text[:200]}...")
                return
                
            except Exception as e:
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Send in bulk toZepfail (  {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Send in bulk toZepfail， {self.MAX_RETRIES}Second-rate: {e}")
    ...
    def _flush_remaining(self):
        ...
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"send{display_name}  {len(buffer)}  ")
