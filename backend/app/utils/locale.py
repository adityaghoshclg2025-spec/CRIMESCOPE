...
def get_locale() -> str:
    if has_request_context():
        raw = request.headers.get('Accept-Language', Config.DEFAULT_LOCALE)
        return raw if raw in _translations else Config.FALLBACK_LOCALE
    return getattr(_thread_local, 'locale', Config.DEFAULT_LOCALE)


def t(key: str, **kwargs) -> str:
    locale = get_locale()
    messages = _translations.get(locale, _translations.get(Config.FALLBACK_LOCALE, {}))
    ...
    if value is None:
        value = _translations.get(Config.FALLBACK_LOCALE, {})
        for part in key.split('.'):
            if isinstance(value, dict):
                value = value.get(part)
            else:
...
def get_language_instruction() -> str:
    locale = get_locale()
    lang_config = _languages.get(locale, _languages.get(Config.FALLBACK_LOCALE, {}))
